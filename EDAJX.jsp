...
<jsp:useBean id="source" class="gov.state.nextgen.presentation.view.pageelements.Source" scope="application"/>
...
function disableWithdrawCloseSw(){
	...
	if(progStatus!=null && progStatus!=undefined && (progStatus=='TN'|| progStatus=='DN') && caseMode=='RE' )
	{
...
if(dcCaseProgramCargo.getProgSrcCd()!=null){
	programSourceCd = dcCaseProgramCargo.getProgSrcCd();
}
  ...
  // String gsFSjs = "validate = 'isComplete("+grandParentSubsidySw.LABEL+"~document.form1."+grandParentSubsidySw.FIELD_NAME+")'";
   ...
   overridePMdisable = "overrideEnableOnNo()";
	...
	onLdDisWithdrawJS = ";disableWithDrawSw()";
		...
		onLoadJS = "onLoad=\"disableTabs('tab4');disableTabs('tab5');"+ priorDtJS.toString() +";" + subsidyChk + ";clearRecFields();enableRein();disableWithdrawFields();disableRevertDate();validateTanfLabels();enableReactivation();disableLiheap();disableOtherTabs();snapOnly();enableDisableApplicationStatDt();disableWithdrawCloseSw();";
	...
	//CH-955:Start:Ankit:Remove call to filterWithdrawClosureReason,enableDisableMedicaidType();;enableDisableDhs3043() : 
	   validatePrg = "onChange = 'checkPgm();tanfLabels();" + subsidyChk + ";snapOnly();' ";
...
<td  id="ToaDescId"> <TL:FWRTSelectTag referenceTableName="PROGRAM" bean="<%=programs%>" filter="YES" filterColumn="ACTIVESW" elementId="4" filterValue="Y" multiple="N" emptySpace="Y" tabindex="9" selected="<%=progCd%>" size="5" other="<%=validatePrg%>"
 ...
 <td><TL:FWLabelTag bean="<%=waitListScreeningRequest%>" elementId="10"  />
 			...
 			<table width="723" title="Case Information" summary="Case Information" border="0" class="bodyFormLabels">	
 	<tr> 	  	    	  
<td><TL:FWLabelTag elementId="16" bean="<%=source%>"  required="Y"/>
                </td>
                <td><TL:FWRTSelectTag elementId="16" referenceTableName="APPMODE" bean="<%=source%>" multiple="N" emptySpace="Y" tabindex="2" selected="<%=programSourceCd%>" size="5"   /> 
                </td>
                </tr>	           
                </table>	
...
<td > <TL:FWGroupCustomDateTag  
	  	 ...
	  	 var addedFlag=false;
	  	   ...
	  	   function onloadRulesForRR(){
	  		   ...
	  		   //Disable Source
				<%
	  		   if("true".equals((String) request.getAttribute("disableSourceFeild"))){
	  			   %>
...
//richa start
function snapOnly(){
	if(document.form1.programs.value!='FS'){
		$("#requestType option[value='NSH']").remove();
		$('#snapText').hide();
		
	}
	else if($("#requestType option:[value=NSH]").length!=1){
		$("#requestType").append('<option value="NSH" title="Not a separate household">Not a separate household</option>');
		$('#snapText').show();
	}
}
//richa end
...
<%-- <% if(null != progCd && progCd.equalsIgnoreCase("FS")){ %> --%>
<table id="snapText">
          <caption>
            <H2 class="Black_Heading_2" > 
            <!-- CR 911 : Changes made in section header -->
            Not a separate household for SNAP 
            </H2>
          </caption>
</table>
<%-- <% } %> --%>
